...
function Utils.dumpTable(t, indent, visited)
    indent = indent or ""
    visited = visited or {}
    ...
    for k, v in pairs(t) do
        ...
        if valType == "table" then
            output = output .. indent .. keyStr .. ":\n"
            output = output .. Utils.dumpTable(v, indent .. "  ", visited)
        elseif valType == "function" or valType == "userdata" or valType == "thread" then
            output = output .. indent .. keyStr .. ": <" .. valType .. ">\n"
        else
            output = output .. indent .. keyStr .. ": " .. tostring(v) .. "\n"
        end
    end
...
function Utils.getCardData(card)
    ...
    if card.edition then
        _card.edition = card.edition.type
    end

    --local card_debug_str = Utils.dumpTable(card)

    --local f = io.open("card_dump.txt", "w")
    --if f then
    --    f:write(card_debug_str)
    --    f:close()
    --end
...
function Utils.getShopData()
    ...
    if G.pack_cards and G.pack_cards.cards then
        for i = 1, #G.pack_cards.cards do
            _shop.pertu[i] = Utils.getCardData(G.pack_cards.cards[i])
        end
    end
